...
const WillGenerator = ({ subscription }) => {
  const location = useLocation();
  const totalSteps = 19; // Add this constant for step tracking
  ...
  const getData = async () => {
    try {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.user.email == mail) {
            setFormData(response.data.user.wills);
          }
        })
        .catch((error) => {
          console.error(
  ...
  const checkSavedData = () => {
    ...
    console.log(
      ...
      saved ? JSON.parse(saved) : "No data saved"
    );
  };
    ...
    try {
      ...
      const willData = {
        ...
        createdAt: isEditMode
          ? savedWills.find((w) => w.id === willId)?.createdAt
      ...
      if (isEditMode) {
        ...
        localStorage.setItem(
          "savedWills",
          JSON.stringify([...savedWills, willData])
        );
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      link.href = url;
      link.download = `Last_Will_and_Testament_${
        formData.testatorName || "Document"
      }.pdf`;
      ...
      <div className="mt-6">
        <div className="mb-4">
          <label className="block mb-4 text-gray-600">
            Identifying all of your grandchildren is optional. It allows you to
            choose them later in this wizard if you decide to leave them some of
            your assets. If you do not plan on leaving anything specific to your
            grandchildren, you can simply skip this page.
          </label>
          ...
          <div className="mb-4">
            <label className="flex items-center space-x-3">
              <input
                ...
                onChange={(e) =>
                  handleInputChange(e, null, "grandchildrenInclusion")
                }
                className="form-radio"
              />
              <span>
                I have grandchildren, but I am NOT leaving them something
                directly in my Will
              </span>
            </label>
          </div>
          ...
          <div className="mb-4">
            <label className="flex items-center space-x-3">
              ...
              />
              <span>
                I have grandchildren, and I MIGHT OR MIGHT NOT leave them
        ...
        {formData.grandchildrenInclusion === "mightLeave" && (
          <>
            {formData.grandchildren.map((grandchild, index) => (
              <div
                key={index}
                className="p-6 border rounded-lg shadow-sm space-y-4 mb-6"
              >
                <h3 className="text-xl font-semibold mb-4">
                  {index === 0 ? "Grandchild's" : `#${index + 1} Grandchild's`}{" "}
                  Details
                </h3>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={grandchild.fullName || details.fullName}
                    onChange={(e) => {
                      setDetails({
                        ...details,
                  ...
                  <div className="space-x-4">
                    <label className="inline-flex items-center">
                      <input
                        ...
                        value="grandson"
                        checked={details.relationship === "grandson"}
                        onChange={(e) =>
                          setDetails({
                            ...details,
                    ...
                    <label className="inline-flex items-center">
                      <input
                        ...
                        name={`relationship-${index}`}
                        value="granddaughter"
                        checked={details.relationship === "granddaughter"}
                        onChange={(e) =>
                          setDetails({
                            ...details,
                            relationship: e.target.value,
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={grandchild.dateOfBirth || details.dateOfBirth}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    type="email"
                    value={grandchild.email || details.email}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={grandchild.occupation || details.occupation}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={grandchild.address || details.address}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={grandchild.parish || details.parish}
                    onChange={(e) =>
                      setDetails({ ...details, parish: e.target.value })
                    }
                    className="w-full p-2 border rounded"
                  />
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    setFormData((prev) => ({
                      ...prev,
                      grandchildren: [details, ...prev.grandchildren],
                    }));
                    setDetails({
                      fullName: "",
                      relationship: "",
                  ...
                  className="bg-blue-500 mt-3 ml-5 text-white px-4 py-2 rounded hover:bg-blue-600"
                >
                  Add Grandchild
                </button>
              </div>
  ...
  const BeneficiarySelect = ({ possession, index, onUpdate }) => {
    ...
    const [localPossession, setLocalPossession] = useState(possession);
      ...
      <div className="space-y-4">
        ...
        <div className="flex space-x-2">
          <select
            ...
            onChange={(e) => {
              const selectedBeneficiary = allBeneficiaries.find(
          ...
          >
            ...
            {formData.children?.length > 0 && (
              <optgroup label="Children">
                {formData.children.map((child, idx) => {
                  const isSelected = possession.beneficiaries?.some(
                    (b) => b.id === `child-${child.fullName}`
                  );
                  if (!isSelected) {
                    return (
                      <option
                        key={`child-${idx}`}
                        value={`child-${child.fullName}`}
                      >
            ...
            {formData.grandchildren?.length > 0 && (
              <optgroup label="Grandchildren">
                {formData.grandchildren.map((grandchild, idx) => {
                  const isSelected = possession.beneficiaries?.some(
                    (b) => b.id === `grandchild-${grandchild.fullName}`
                  );
                  if (!isSelected) {
                    return (
                      <option
                        key={`grandchild-${idx}`}
                        value={`grandchild-${grandchild.fullName}`}
                      >
                        {grandchild.fullName} (Grandchild -{" "}
            ...
            {formData.additionalBeneficiaries?.length > 0 && (
              <optgroup label="Other Beneficiaries">
                {formData.additionalBeneficiaries.map((ben, idx) => {
                  const isSelected = possession.beneficiaries?.some(
                    (b) => b.id === `other-${ben.fullName}-${idx}`
                  );
                  if (!isSelected) {
                    return (
                      <option
                        key={`ben-${idx}`}
                        value={`other-${ben.fullName}-${idx}`}
                      >
          ...
          <div className="mt-4">
            ...
            <div className="space-y-4">
              {possession.beneficiaries.map((beneficiary, idx) => (
                <div
                  key={beneficiary.id}
                  className="p-4 bg-gray-50 rounded-lg relative border"
                >
                  <button
                    type="button"
                    onClick={() => {
                      onUpdate(index, {
                        ...possession,
                        beneficiaries: possession.beneficiaries.filter(
                          (_, i) => i !== idx
                        ),
                      });
                    }}
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updatedBeneficiaries =
                            possession.beneficiaries.map((b, i) =>
                              i === idx
                                ? {
                                    ...b,
                                    sharePercentage:
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                <div className="font-medium">
                  Total Share:{" "}
                  {possession.beneficiaries.reduce(
                    (total, ben) => total + (ben.sharePercentage || 0),
                    0
                  )}
                  %
                </div>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-8 rounded-lg shadow-lg">
          ...
          <div className="bg-white p-8 rounded-xl shadow-md mb-8">
            <div className="flex flex-col items-center mb-6">
              ...
              <h3 className="text-2xl font-semibold text-gray-800 mb-2">
                Save Your Will Details
              </h3>
              <p className="text-gray-600 text-center max-w-md mb-6">
                Secure all your important information with just one click. You
                can always come back and modify it later.
              </p>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600 text-sm">
                Return anytime to modify your details
              </p>
            </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  Remember to save your progress regularly. Your information is
                  valuable - keep it safe!
                </p>
              </div>
            </div>
          </div>
  ...
  const navigateForward = () => {
    ...
    if (currentStep === 8) {
      const minorChildren = formData.children.filter(
        (child) => child.dateOfBirth && calculateAge(child.dateOfBirth) < 18
      );

      if (formData.guardians !== undefined) {
        formData.guardians.forEach((element) => {
      ...
      if (
        (minorChildren.length === 0 &&
          formData.livingGrandchildren !== "yes") ||
  ...
  const formatPossessionForPDF = (possession) => {
    const formatBeneficiaryDetails = (beneficiaries) => {
      ...
      return beneficiaries
        .map((b) => {
          ...
          return (
            `${b.fullName}${typeLabel} of ${b.address}${
              b.parish ? `, ${b.parish}` : ""
    ...
    const beneficiaryDetails = formatBeneficiaryDetails(
      possession.beneficiaries
    );
    ...
    switch (possession.type) {
      case "Property":
        return (
          `Property- situate at ${possession.address}, in the parish of ${possession.parish} ` +
          `registered at Volume ${possession.volume} and Folio ${possession.folio} of the Register ` +
          `Book of Titles to be shared between ${
      ...
      case "Shares and Stocks":
        return (
          `Shares in ${possession.company} held in ${possession.country} at ${possession.exchange} ` +
      ...
      case "Insurance":
        return (
          `Proceeds of insurance policy numbered ${possession.policyNumber}, held at ${possession.company} ` +
          `located at ${possession.address}, ${possession.country}, being a ${possession.policyType} policy ` +
          `with sum insured of ${possession.sumInsured} to be shared between ${
            beneficiaryDetails || noBeneficiariesText
          }.`
        );
      ...
      case "Bank Accounts":
        return (
          `Proceeds of ${possession.accountType || ""} account numbered ${
            possession.accountNumber
          }, ` +
          `held at ${possession.bank} located at ${possession.address}, ${possession.country}, ` +
          `in ${possession.currency || ""} currency to be shared between ${
            beneficiaryDetails || noBeneficiariesText
          }.`
        );
      ...
      case "Motor Vehicle":
        return (
          `${possession.color} ${possession.year} ${possession.make} ${possession.model} Motor vehicle ` +
          `bearing License plate number ${possession.licensePlate} and engine number ${possession.engineNumber} ` +
          `and chassis number ${
            possession.chassisNumber
          }, currently located at ${possession.parkingLocation || ""} ` +
          `to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );

      case "Unpaid Salary":
        return (
          `Unpaid salary and/or emoluments with my employer, ${possession.employer} ` +
          `located at ${possession.employerAddress}, Employee ID: ${
            possession.employmentId || ""
          }, ` +
          `covering the period ${
            possession.periodCovered || ""
          } to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );

      case "NHT Contributions":
        return (
          `Refund of National Housing Trust Contributions (NHT Number: ${possession.nhtNumber}, ` +
          `Tax Number: ${possession.taxNumber}), with total contributions of ${
            possession.totalContributions || ""
          } ` +
          `up to ${
            possession.lastContributionDate || ""
          } to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );

      case "Jewellery":
        return (
          `Jewellery described as ${possession.description}, being a ${
            possession.itemType || ""
          } ` +
          `made of ${possession.material || ""}${
            possession.stones ? `, set with ${possession.stones}` : ""
          }, ` +
          `valued at ${possession.appraisalValue || ""}, currently located at ${
            possession.location || ""
          } ` +
          `to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );
        ...
        return (
          `Furniture ${
            possession.description
              ? `described as ${possession.description}, `
              : ""
          }` +
          `currently located at ${possession.location || ""}, ` +
          `valued at ${possession.estimatedValue || ""} to be shared between ${
            beneficiaryDetails || noBeneficiariesText
          }.`
        );
      ...
      case "Paintings":
        return (
          `Painting titled "${possession.title || ""}" by artist ${
            possession.artist || ""
          }, ` +
          `created in ${possession.yearCreated || ""}, ${
            possession.medium || ""
          }, ` +
          `measuring ${possession.dimensions || ""}, currently located at ${
            possession.location || ""
          }, ` +
          `valued at ${possession.appraisalValue || ""} to be shared between ${
            beneficiaryDetails || noBeneficiariesText
          }.`
        );
      ...
      case "Firearm":
        return (
          `Firearm bearing serial number ${possession.serialNumber} and firearm licence ` +
      ...
      case "Digital Assets":
        return (
          `Digital assets described as ${possession.assetType || ""} held on ${
            possession.platform || ""
          }, ` +
          `account identifier ${
            possession.accountDetails || ""
          }, with estimated value of ${possession.estimatedValue || ""} ` +
          `to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );
      ...
      case "Intellectual Property":
        return (
          `Intellectual Property rights being ${
            possession.type || ""
          } registered under number ${possession.registrationNumber || ""} ` +
          `in ${possession.jurisdiction || ""}, valid until ${
            possession.expiryDate || ""
          }, ` +
          `with associated rights ${
            possession.rightsOwned || ""
          } to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );
      ...
      case "Business Interests":
        return (
          `Business interests in ${
            possession.companyName || ""
          } (Registration Number: ${possession.registrationNumber || ""}), ` +
          `being a ${possession.businessType || ""} with ${
            possession.ownershipPercentage || ""
          }% ownership, ` +
          `valued at ${
            possession.valuationDetails || ""
          } to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );

      case "Residual Estate":
        return (
          `Residual estate comprising ${possession.description || ""}, ` +
          `with estimated value of ${
            possession.estimatedValue || ""
          }, subject to ${possession.specialConditions || ""} ` +
          `to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );
      ...
      default:
        return (
          `${possession.type}: ${
            possession.description || "No description provided"
          } ` +
          `to be shared between ${beneficiaryDetails || noBeneficiariesText}.`
        );
    }
  ...
  const renderAddPossessions = () => {
    const hasPossessions =
      formData.possessions && formData.possessions.length > 0;
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          onClick={() => {
            ...
            const newPossession = createEmptyPossession(
              formData.selectedPossession
            );
            setFormData((prev) => ({
              ...prev,
      ...
      <div className="space-y-4">
        ...
        {possession.beneficiary && (
          <div className="mt-2 p-4 bg-gray-50 rounded-lg">
            ...
            {possession.beneficiaryParish && (
              <p>Parish: {possession.beneficiaryParish}</p>
            )}
            {possession.beneficiaryRelationship && (
              <p>Relationship: {possession.beneficiaryRelationship}</p>
            )}
          </div>
        )}
      </div>
  ...
  const transformFormDataForPDF = (data) => {
    return {
      ...
      possessions: {
        ...
        vehicles:
          data.motorVehicles?.map((vehicle) => ({
            color: vehicle.color || "",
            make: vehicle.make || "",
            model: vehicle.model || "",
  ...
  const removeBeneficiary = (index) => {
    setFormData((prev) => ({
      ...prev,
      additionalBeneficiaries: prev.additionalBeneficiaries.filter(
        (_, i) => i !== index
      ),
    }));
  };
    ...
    try {
      ...
      const formatPossessionForPDF = (possession) => {
            ...
            if (possession.beneficiary) {
              return (
                `to ${possession.beneficiary}${
                  possession.beneficiaryType === "Child" ? " (Child)" : ""
                } ` +
                `${
                  possession.beneficiaryRelationship
                    ? `(${possession.beneficiaryRelationship})`
                ...
                `of ${possession.beneficiaryAddress}${
                  possession.beneficiaryParish
                    ? `, ${possession.beneficiaryParish}`
                    : ""
                }`
        ...
        switch (possession.type) {
          ...
          case "Bank Accounts":
            return (
              `Proceeds of bank account numbered ${possession.accountNumber}, held at ${possession.bank} ` +
              `located at ${possession.address}, ${possession.country} ${beneficiaryDetails}.`
            );

          case "Motor Vehicle":
            return (
              `${possession.color} ${possession.make} ${possession.model} Motor vehicle bearing License ` +
          ...
          case "Unpaid Salary":
            return (
              `Unpaid salary and/or emoluments with my employer, ${possession.employer} located at ` +
      ...
      const personalInfo = `THIS IS THE LAST WILL AND TESTAMENT of me ${
        formData.prefix || ""
      } ${formData.testatorName || ""} ${formData.suffix || ""}, a ${
        formData.occupation || ""
      ...
      formData.executors?.forEach((executor, index) => {
        if (
          ...
          const executorText = `I HEREBY APPOINT ${executor.name || ""}, my ${
            executor.relationship || ""
          }, ${executor.occupation || ""}, of ${
            executor.address || ""
          }, in the parish of ${executor.parish || ""}${
            index < formData.executors.length - 1 ? " AND" : ""
          }`;
      ...
      Object.entries(possessionTypes).forEach(([type, letter]) => {
        const possessionsOfType = formData.possessions.filter(
          (p) => p.type === type
        );
      ...
      yOffset = writeText(
        currentPage,
        `SIGNED by Testator to said ${formData.testatorName}, a ${formData.occupation} of ${formData.address}, in the parish of ${formData.parish}, as my Last Will and Testiment I declare that I have signed`,
        {
          y: yOffset,
        }
      );
  ...
  const downloadPDF = async (formData) => {
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `Last_Will_and_Testament_${formData.testatorInfo.fullName.replace(
        /\s+/g,
        "_"
      )}.pdf`;
    ...
    switch (currentStep) {
        ...
        return (
          <section className="introduction bg-gray-100 p-8 rounded-lg shadow-lg max-w-3xl mx-auto mt-10">
            <h2 className="text-3xl font-bold text-blue-700 mb-6 text-center">
              Welcome to Your Last Will and Testament
            </h2>
            <p className="text-lg text-gray-700 mb-4 text-center">
              We've made this process as easy and quick as possible. It should
              only take a few minutes of your time.
            </p>
            <div className="mb-6">
              <p className="text-gray-600 mb-3">
                You will be asked a series of questions to guide you through
                creating your Last Will and Testament.
              </p>
              <p className="text-gray-600 mb-3">
                For general help, refer to the "Common Questions" that are
                available on each page. If you can't see them, click on the
                large "?" at the top of the page.
              </p>
              <p className="text-gray-600 mb-3">
                For specific help with any question, hover over or tap the small
              ...
              <p className="text-gray-600 mb-4">
                Once you're finished, make sure to print and sign your document
                in front of witnesses to make it legally binding.
              </p>
            </div>
                ...
                {["Yes", "No"].map((option) => (
                  <label key={option} className="inline-flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(e, null, "livingChildren")
                      }
                    />
                ...
                {["Yes", "No"].map((option) => (
                  <label key={option} className="inline-flex items-center">
                    <input
                      ...
                      value={option.toLowerCase()}
                      checked={
                        formData.livingGrandchildren === option.toLowerCase()
                      }
                      onChange={(e) =>
                        handleInputChange(e, null, "livingGrandchildren")
                      }
                    />
                    <span className="ml-2">{option}</span>
                  </label>
      ...
      case 4:
        return (
          <section>
            <h2 className="text-2xl font-semibold mb-4">
              Spouse/Partner Details if you are Single / Divorced any other
              reason you can skip this part
            </h2>
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(e, "children", "fullName", index)
                      }
                      className="w-full p-2 border rounded"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-x-4">
                      {["Son", "Daughter", "Gender Neutral Child"].map(
                        (rel) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(e, "children", "dateOfBirth", index)
                      }
                      className="w-full p-2 border rounded"
                    />
                    {child.dateOfBirth && (
                    ...
                    <>
                      <div>
                        ...
                        <input
                          ...
                          value={child.email}
                          onChange={(e) =>
                            handleInputChange(e, "children", "email", index)
                          }
                          className="w-full p-2 border rounded"
                        />
                      </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={child.address || ""}
                      onChange={(e) =>
                        handleInputChange(e, "children", "address", index)
                      }
                      className="w-full p-2 border rounded"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={child.parish || ""}
                      onChange={(e) =>
                        handleInputChange(e, "children", "parish", index)
                      }
                      className="w-full p-2 border rounded"
      ...
      case 6:
        return (
          <section>
            ...
            <div className="mb-4">
              <label className="block text-gray-700 text-sm font-bold mb-2">
                Do you have any deceased family members?
              </label>
              <div className="flex space-x-4">
                {["Yes", "No"].map((option) => (
                  <label key={option} className="inline-flex items-center">
                    <input
                      ...
                      value={option.toLowerCase()}
                      checked={
                        formData.hasDeceasedFamilyMembers ===
                        option.toLowerCase()
                      }
                      onChange={(e) =>
                        handleInputChange(e, null, "hasDeceasedFamilyMembers")
                      }
                    />
                    <span className="ml-2">{option}</span>
                  </label>
                ...
                {formData.deceasedFamilyMembers.map((member, index) => (
                  <div key={index} className="mb-4 p-4 border rounded">
                    <h3 className="text-xl font-semibold mb-2">
                      Deceased Family Member {index + 1}
                    </h3>
                    {renderInput(
                      "deceasedFamilyMembers",
                      "fullName",
                ...
                <button
                  ...
                  onClick={() => {
                    setFormData({
                      ...formData,
                      deceasedFamilyMembers: [
                        ...formData.deceasedFamilyMembers,
      ...
      case 7:
        return (
          <section className="space-y-6">
            <h2 className="text-2xl font-semibold mb-4">
              Identify Others to be Included in your Will
            </h2>
            <p className="text-gray-600 mb-4">
              If there are other people or organizations to be included in your
              Will, you can name them now. You can also add more names later, as
              you are working through this wizard.
            </p>
            <p className="text-gray-600 mb-4">
              You should not include your spouse/partner, children, or
              grandchildren on this page, because they would have been named in
              previous pages of this wizard.
            </p>
            <p className="text-gray-600 mb-4">
              By listing the beneficiaries here, it makes it easier to select
              them later on for receiving a bequest. You are also able to set up
              a trust for them. They do not appear in your Will unless they are
            ...
            <div className="space-y-4">
              <label className="flex items-center space-x-3">
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange(e, null, "otherBeneficiaries")
                  }
                  className="form-radio"
                />
                <span>
                  I have no other beneficiaries, or will add them later
                </span>
              </label>
      ...
      case 8:
        ...
        return (
          <section className="space-y-6">
            ...
            {(formData.livingChildren === "no" || !minorChildren.length) &&
            formData.livingGrandchildren !== "yes" ? (
              <div className="p-4 bg-gray-100 rounded">
                <p>
                  You have indicated that you have no minor children or
                  grandchildren that need guardians.
                </p>
                      ...
                      <div>
                        <label className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={
                              formData.guardians[index]?.isAdult || false
                            }
                            onChange={(e) => {
                              setFormData((prev) => {
                                const newGuardians = [
              ...
              {[0, 1].map((index) => (
                <div key={index} className="mb-8">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    {renderInput("witnesses", "parish", "Parish", index)}{" "}
                    {/* Added parish field */}
                    {renderInput(
                      "witnesses",
                      "occupation",
          ...
          <section>
            ...
            <div className="mb-4">
              ...
              <select
                ...
                value={formData.selectedPossession || ""}
                onChange={(e) =>
                  handleInputChange(e, null, "selectedPossession")
                }
              >
                <option value="">Select a possession type</option>
                <option value="property">Property</option>
                ...
                <option value="unpaidSalary">
                  Unpaid Salary and Emoluments
                </option>
                <option value="nhtContributions">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-50 p-6 rounded-lg shadow">
                ...
                <dl className="space-y-2">
                  ...
                  <dd className="ml-4 text-gray-600">
                    {formData.address}, {formData.parish}
                  </dd>
                </dl>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-lg shadow">
                ...
                <dl className="space-y-2">
                  ...
                  <dd className="ml-4 text-gray-600">
                    {formData.livingChildren === "yes" ? "Yes" : "No"}
                  </dd>
                  <dt className="font-medium">Living Grandchildren</dt>
                  <dd className="ml-4 text-gray-600">
                    {formData.livingGrandchildren === "yes" ? "Yes" : "No"}
                  </dd>
                </dl>
              </div>
                  ...
                  {formData.possessions.map((possession, index) => (
                    <div key={`possession-${index}`} className="border-b pb-4">
                      ...
                      <div className="ml-4 text-sm text-gray-600">
                        {possession.beneficiaries &&
                          possession.beneficiaries.map((ben, idx) => (
                            <div key={`ben-${idx}`} className="mt-2">
                              <span className="font-medium">
                                Beneficiary {idx + 1}:
                              </span>{" "}
                              {ben.fullName} ({ben.sharePercentage}%)
                            </div>
                          ))}
                      </div>
                    </div>
      ...
      case 19:
        return (
          <section className="space-y-6">
            ...
            <p>
              Review your information and click the button below to generate
              your will.
            </p>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="flex justify-between mt-8 mr-10">
          <button
            type="button"
          ...
          <div className="">
            ...
            {subscription === "yearly" && (
              <button
                ...
                className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-800 absolute ml-5   "
              >
                Exit
              </button>
            )}
