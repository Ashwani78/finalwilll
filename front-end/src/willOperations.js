...
export const saveWillToSupabase = async (formData) => {
  try {
    ...
    if (formData.witnesses?.length > 0) {
      const { error: witnessError } = await supabase.from("witnesses").insert(
    ...
    if (formData.executors?.length > 0) {
      const { error: executorError } = await supabase.from("executors").insert(
...
export const updateWillInSupabase = async (willId, formData) => {
  try {
    ...
    await supabase.from("executors").delete().eq("will_id", willId);
