...
const Dashboard = () => {
  ...
  const [sharewill, setSharewill] = useState(null);
...
const uploadFile = async (file) => {
  ...
  const handleSendEmail = async (email) => {
    if (!fileUrl) {
      console.error("No file URL available to send.");
      return;
    }

    // EmailJS service, template, and user IDs
    const serviceID = "service_28wyrsr";
    const templateID = "template_nr5pnp5";
    const userID = "iAaWmS8NcqlFM2HZq";

    // Prepare the email data
    const emailData = {
      to_email: email, // recipient email
      from_name: "Final Will",
      message: `A will document has been shared with you.
        To access the document:
        Click the link: ${fileUrl}`,
      file_url: fileUrl, // Attach the file URL
    };
  ...
  const ShareWillPop = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">SHARE WILL</h2>
          <button
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
          {sharewill ? (
            <div>
              ...
              >
                <Upload className="w-12 h-12 text-blue-500 mb-2" />
                <p className="text-gray-600">File has been uploaded</p>
                <p className="text-sm text-gray-500 mt-1">
                  If you want to change the file click here
                </p>
              </label>
            </div>
          ) : (
        ...
        <div className="w-full text-black mt-4 flex flex-col">
          {formData?.additionalBeneficiaries.length === 0 ? (
            <div>There are no Beneficiaries</div>
          ) : (
            formData?.additionalBeneficiaries.map((individual, index) => (
              <div key={index} className="flex flex-row justify-between p-4">
                <p>{individual.email}</p>
                <button
                  className="text-blue-600 hover:text-blue-800"
                  onClick={() => {
                    handleSendEmail(individual.email);
                  }}
                >
                  send mail
                </button>
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4">
          <button
            onClick={() =>
              window.open("https://app.pandadoc.com/a/#/dashboard", "_blank")
            }
      ...
      {formData ? (
        ...
        <div className="w-full h-[300px] flex items-center justify-center">
          <h1 className="text-xl text-gray-600">Create a New Will</h1>
        </div>
      )}
